...
// Hier kun je dus dingen blokkeren.
@Component
public class JwtTokenAuthorizationOncePerRequestFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		// Niet expired is,
		// En er een valid username(email) in zit.
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			...
			// Checkt of persoon in dienst is.
			if (!persoonService.vindPersoonUitDienst(username)) {

				// Hier checkt hij of de user bestaat.
				// Haalt de user op username.
				UserDetails userDetails = this.persoonService.loadUserByUsername(username);

				// Checkt of de token inderdaad de username bevat.
				// Vervolgens haalt hij alle authorities op die gebruikt worden in de
				...
				if (jwtTokenUtil.validateToken(jwtToken, userDetails)) {
					...
					// Wanneer een user is authenticated wordt hij in de SecurityContextHolder
					// geplaatst.
				...
				logger.error("Email niet in database");
			}
		}
